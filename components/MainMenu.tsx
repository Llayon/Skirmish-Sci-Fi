...
const ContinuePanel: React.FC<{ slot: SaveSlot; onContinue: () => void }> = ({
  slot,
  onContinue,
}) => {
  ...
  return (
    <div
      className="main-menu-panel-container opacity-0 animate-[fade-in-down_0.5s_ease-out_forwards]"
      style={{ animationDelay: "0.6s" }}
    >
      <div className="main-menu-panel">
        <div className="space-y-6">
          <h2 className="font-orbitron text-2xl text-primary">
            {t("buttons.continue")}
          </h2>
          <div className="space-y-4 text-left">
            <div className="flex items-center gap-4">
              ...
              <span className="font-semibold text-text-base">
                {sanitizeToText(slot.metadata.crewName)}
              </span>
            </div>
            <div className="flex items-center gap-4">
              <Calendar className="w-5 h-5 text-primary/80" />
              <span className="text-text-base">
                {t("dashboard.campaignTurn")}: {slot.metadata.turn}
              </span>
            </div>
            <div className="flex items-center gap-4">
              <Coins className="w-5 h-5 text-primary/80" />
              <span className="text-text-base">
                {t("crewCreator.startingCredits")}: {slot.campaign.credits}
              </span>
            </div>
          </div>
          <button
            onMouseEnter={preloadCampaignDashboard}
            onClick={onContinue}
            className="sci-fi-button w-full text-lg"
          >
            <Play className="w-5 h-5 mr-2" />
            {t("buttons.continue")} Campaign
          </button>
          <p className="text-xs text-text-muted text-center">
            {t("saveSlots.lastSaved")}: {formattedDate}
          </p>
        </div>
    ...
    >
      <div className="main-menu-panel">
        <div className="space-y-4 text-center">
          <h2 className="font-orbitron text-2xl text-primary">
            {t("mainMenu.sagaAwaits")}
          </h2>
          <p className="text-text-base">{t("mainMenu.welcome")}</p>
          <button
            onMouseEnter={preloadCrewCreator}
            onClick={onNewGame}
            className="sci-fi-button w-full text-lg"
          >
    ...
    >
      <div className="main-menu-panel">
        <div className="space-y-2 text-center">
          <h3 className="font-orbitron text-lg text-primary/90 flex items-center justify-center gap-2">
            <Radio size={16} /> {t("mainMenu.fringeIntel")}
          </h3>
...
const MainMenu: React.FC = () => {
  ...
  const lastSaveSlot = useMemo(() => {
    const slots = Object.values(saveSlots).filter(
      (slot): slot is SaveSlot => !!slot,
    );
  ...
  const handleContinue = () => {
    if (lastSaveSlot) {
      const slotId = Object.keys(saveSlots).find(
        (key) => saveSlots[key as keyof typeof saveSlots] === lastSaveSlot,
      );
      if (slotId) {
  ...
  const menuItems = [
    ...
    {
      id: "load_game",
      label: t("buttons.loadGame"),
      icon: FolderOpen,
      action: () => setLoadModalOpen(true),
      onMouseEnter: preloadLoadGameModal,
      condition: hasSaves,
    },
    {
      id: "quick_battle",
      label: t("quickBattle.title"),
      icon: Zap,
    ...
    {
      id: "multiplayer",
      label: t("buttons.playWithFriend"),
      icon: Users,
      action: handleMultiplayer,
      onMouseEnter: preloadMultiplayerLobby,
      condition: true,
    },
    {
      id: "settings",
        ...
        <div className="relative z-20 w-full max-w-7xl p-6 lg:p-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="flex flex-col justify-center space-y-12 text-center lg:text-left lg:items-start">
            ...
            <div className="flex items-center justify-center lg:justify-start gap-8">
              <div
                className="menu-decorator hidden lg:block opacity-0 animate-[fade-in-down_0.5s_ease-out_forwards]"
                style={{ animationDelay: "0.5s" }}
              >
                <div
                  className="absolute top-0 left-0 w-full h-full flex flex-col justify-around items-center text-primary/50 font-mono text-[10px] pointer-events-none opacity-0 animate-[fade-in-down_0.5s_ease-out_forwards]"
            ...
            <footer
              className="text-text-muted text-xs opacity-0 animate-[fade-in-down_0.5s_ease-out_forwards]"
              style={{ animationDelay: "1.2s" }}
            >
              {t("mainMenu.version")}
            </footer>
