...
const { useBattleStore, useCampaignProgressStore, useMultiplayerStore } = await import('../../stores');

describe('ReactionRollPanel', () => {
  const mockBattleActions = { dispatchAction: vi.fn(), advancePhase: vi.fn(), setBattle: vi.fn() };
  const mockState = {
    battle: { reactionRolls: {}, reactionRerollsUsed: false, participants: [], difficulty: 'normal' },
    actions: mockBattleActions,
  };
  ...
  it('renders Roll and Seize buttons in solo mode', () => {
    vi.mocked(useBattleStore).mockImplementation((selector: any) => selector(mockState));
    vi.mocked(useCampaignProgressStore).mockReturnValue({ campaign: { storyPoints: 1 }, actions: {} } as any);
