...
export const useBattleAutomations = (scrollContainerRef: React.RefObject<HTMLDivElement>) => {
    ...
    const engineV2Enabled = useBattleStore(state => state.engineV2Enabled);
    const { 
        endTurn, 
        processEnemyTurn, 
        advancePhase, 
        ...
        setIsProcessingEnemies, 
        setShowEnemyTurnBanner,
        dispatchEngineAction
    } = useBattleStore(state => state.actions);

    // Helper to wait for V2 animations/events to finish
    const waitForV2Events = async () => {
        let hasEvents = true;
        while (hasEvents) {
            await sleep(100);
            const state = useBattleStore.getState();
            hasEvents = state.eventCursor < state.events.length;
        }
    };
        ...
        if (battle?.phase === 'enemy_actions' && !multiplayerRole && !isProcessingEnemies) {
            const processTurns = async () => {
                ...
                for (const enemyId of enemiesToAct) {
                    ...
                    if (engineV2Enabled) {
                        // V2: Dispatch atomic turn processing
                        dispatchEngineAction({ type: 'PROCESS_AI_TURN', participantId: enemyId });
                        // Wait for UI to consume all events generated by the AI turn
                        await waitForV2Events();
                    } else {
                        // V1: Legacy async processing
    ...
    }, [battle?.phase, multiplayerRole, isProcessingEnemies, battle?.enemyTurnOrder, advancePhase, processEnemyTurn, setAnimation, setAnimatingParticipantId, clearAnimation, setSelectedParticipantId, setIsProcessingEnemies, setShowEnemyTurnBanner, engineV2Enabled, dispatchEngineAction]);
