...
import { Character, Injury, BattleParticipant, StatUpgrade, Campaign, Crew, CharacterStats, AdvancedTrainingId } from '@/types';
...
const PostBattleSequence: React.FC = () => {
    ...
    const [warResults, setWarResults] = useState<Record<string, { key: string, params?: any }>>({});
    ...
    const handleResolveWarProgress = (planetName: string) => {
        const result = campaignActions.resolveGalacticWarProgress(planetName);
        setWarResults(prev => ({ ...prev, [planetName]: result }));
    };
    ...
    const renderStepContent = () => {
        switch(currentStep.key) {
                                ...
                                {contestedPlanets.map(planet => (
                                    <div key={planet.name} className="p-2 bg-surface-base/50 rounded-md">
                                        ...
                                        {warResults[planet.name] && (
                                            <p className="mt-2 text-sm text-primary italic text-center animate-fade-in">
                                                {t(warResults[planet.name].key, warResults[planet.name].params)}
                                            </p>
                                        )}
                                    </div>
                                ))}
            ...
            case 'purchase':
                ...
                const basicWeaponsToSell = ['scrap_pistol', 'blade', 'colony_rifle', 'shotgun'];
                const allItems = [
                    ...
                    <div className="w-full max-w-2xl mx-auto text-left space-y-4">
                        ...
                        <div className="p-3 bg-surface-raised/50 rounded-md">
                            ...
                            <div className="flex flex-wrap gap-2">
                                <Button onClick={() => handlePurchaseRoll('military')} disabled={campaign.credits < 3}>{t('postBattle.purchase.militaryWeapon')}</Button>
                                <Button onClick={() => handlePurchaseRoll('gear')} disabled={campaign.credits < 3}>{t('postBattle.purchase.gear')}</Button>
                                <Button onClick={() => handlePurchaseRoll('gadget')} disabled={campaign.credits < 3}>{t('postBattle.purchase.gadget')}</Button>
                            </div>
                        ...
                        <div className="p-3 bg-surface-raised/50 rounded-md">
                             ...
                             <div className="flex flex-wrap gap-2">
                                {basicWeaponsToSell.map(wId => (
                                    <Button key={wId} onClick={() => campaignActions.purchaseBasicWeapon(wId)} disabled={campaign.credits < 1}>
                                        {t(`weapons.${wId}`)} (1cr)
                                    </Button>
                                ))}
                             </div>
    ...
    return (
        <div className="max-w-3xl mx-auto">
            <Card>
                <h2 className="text-3xl font-bold font-orbitron text-center mb-2 text-primary">
                    {t('postBattle.title')}
                </h2>
                ...
                <Card className="bg-surface-base/50 min-h-[200px] flex items-center justify-center p-4">
                    <div className="text-center w-full">
                        <h3 className="text-xl font-bold font-orbitron text-primary mb-4">{t(currentStep.titleKey)}</h3>
                        {renderStepContent()}
                    </div>
                </Card>
