import { Battle, BattleParticipant, LogEntry, MissionType, isProtectMission, isEliminateMission, isAcquireMission, isDeliverMission, isMoveThroughMission, isPatrolMission, isSecureMission } from '../../types';
...
export function checkMissionStatus(battle: Battle, context: 'after_action' | 'end_of_round'): { battle: Battle, logs: LogEntry[] } {
    ...
    if ((isAcquireMission(mission) || isDeliverMission(mission)) && mission.itemDestroyed) {
        mission.status = 'failure';
        battle.phase = 'battle_over';
        return { battle, logs };
    }
    ...
    if (context === 'after_action') {
        ...
        } else if (isEliminateMission(mission)) {
        ...
        } else if (isMoveThroughMission(mission)) {
    ...
    if (context === 'end_of_round' && mission.status === 'in_progress') {
        ...
        if (mission.status === 'in_progress') {
            ...
            } else if (isProtectMission(mission)) {
                 if (gridSize) {
            ...
            } else if (isSecureMission(mission)) {
                 if (gridSize) {
                     const center = { x: Math.floor(gridSize.width / 2), y: Math.floor(gridSize.height / 2) };
                     ...
                     if (crewInZone && !enemiesNear) {
                         ...
                         if ((mission.secureRoundsCompleted || 0) > 0) {
