...
import {
  UNIQUE_INDIVIDUALS_TABLE,
  ENEMY_ENCOUNTER_CATEGORY_TABLE,
  CRIMINAL_ELEMENTS_SUBTABLE,
  HIRED_MUSCLE_SUBTABLE,
  INTERESTED_PARTIES_SUBTABLE,
  ROVING_THREATS_SUBTABLE,
  RulebookEnemyTemplate,
...
import {
  ...
  EngineBattleState,
} from "../engine/battle/types";
import {
  DEPLOYMENT_CONDITIONS_TABLE,
  DeploymentConditionEntry,
...
import {
  BASIC_ENEMY_WEAPON_TABLE,
  OPPORTUNITY_MISSION_TABLE,
  PATRON_MISSION_TABLE,
  QUEST_MISSION_TABLE,
  RIVAL_ATTACK_TABLE,
  SPECIALIST_WEAPON_TABLE,
} from "../../constants/battleSetup";
...
const PORTRAITS = Array.from(
  { length: 30 },
  (_, i) =>
    `/assets/portraits/sci_fi_portrait_${String(i + 1).padStart(2, "0")}.png`,
);
let portraitIndex = 0;
...
export const setupBattle = async (
  ...
  const missionTemplate =
    MISSION_DEFINITIONS.find((m) => m.type === finalMissionType) ??
  ...
  const rivalIdForBattle =
    campaign?.attackingRivalId || campaign?.locatedRivalId;
  ...
  const deploymentCondition: DeploymentCondition | undefined = conditionEntry
    ? {
        id: conditionEntry.id,
        nameKey: conditionEntry.nameKey,
        descriptionKey: conditionEntry.descriptionKey,
      }
    : undefined;
  ...
  switch (category) {
    case "Hired Muscle":
      subtable = HIRED_MUSCLE_SUBTABLE;
      break;
    case "Interested Parties":
      subtable = INTERESTED_PARTIES_SUBTABLE;
      break;
    case "Roving Threats":
      subtable = ROVING_THREATS_SUBTABLE;
      break;
    case "Criminal Elements":
    default:
      subtable = CRIMINAL_ELEMENTS_SUBTABLE;
      break;
  }
  ...
  if (
    campaign?.currentWorld?.traits.some((t) => t.id === "heavily_enforced") &&
  ...
  if (
    campaign?.currentWorld?.traits.some((t) => t.id === "rampant_crime") &&
    category === "Criminal Elements"
  ) {
  ...
  if (
    campaign?.currentWorld?.traits.some((t) => t.id === "dangerous") &&
  ...
  for (let i = 0; i < numOpponents; i++) {
    const enemy: Enemy = {
      ...
      isFearless:
        enemyTemplate.ai === "Rampaging" ||
        (enemyTemplate.panic[0] === 0 && enemyTemplate.panic[1] === 0),
  ...
  if (hasLieutenant) {
    ...
    if (nonSpecialists.length > 0) {
      ...
      nonSpecialists[0].weapons.push({
        instanceId: `lw_${nonSpecialists[0].id}`,
        weaponId: "blade",
      });
    }
  }
  ...
  if (enemyTemplate.predefinedWeapons) {
    enemies.forEach((e) => {
      e.weapons.push(
        ...enemyTemplate.predefinedWeapons!.map((wId) => ({
          instanceId: `pw_${e.id}_${wId}`,
          weaponId: wId,
        })),
      );
    });
    ...
    const [, weaponCol, specCol] = (enemyTemplate.weaponsCode || "1 A").match(
      /(\d+)\s+([A-C])/,
    ) || ["1 A", "1", "A"];
    ...
    nonSpecialists.forEach((e, i) => {
      ...
      if (weaponEntry) {
        const weaponIds = weaponEntry[
          `col${weaponCol}` as keyof typeof weaponEntry
        ] as string[];
        e.weapons.push(
          ...weaponIds.map((wId) => ({
            instanceId: `bw_${e.id}_${wId}`,
            weaponId: wId,
          })),
        );
      }
    ...
    specialists.forEach((e) => {
      ...
      if (weaponEntry) {
        const weaponIds = weaponEntry[
          `col${specCol}` as keyof typeof weaponEntry
        ] as string[];
        e.weapons.push(
          ...weaponIds.map((wId) => ({
            instanceId: `sw_${e.id}_${wId}`,
            weaponId: wId,
          })),
        );
      }
  ...
  if (addUnique) {
    for (let i = 0; i < numUniques; i++) {
      const uniqueEntry = resolveTable(
        UNIQUE_INDIVIDUALS_TABLE,
        rollD6() + rollD6(),
      );
      const uniqueStats = { ...enemyTemplate };
      // Apply modifiers
      if (uniqueEntry.speed !== "same")
        uniqueStats.speed = parseInt(uniqueEntry.speed.replace('"', ""), 10);
      if (uniqueEntry.combat.includes("+"))
        uniqueStats.combat += parseInt(
          uniqueEntry.combat.replace("same", "0"),
          10,
        );
      if (uniqueEntry.toughness.includes("-"))
        uniqueStats.toughness -= parseInt(
          uniqueEntry.toughness.replace("same", "0").replace("-", ""),
          10,
        );

      const unique: Enemy = {
        id: `unique_${Date.now()}_${i}`,
        name: uniqueEntry.id,
        isUnique: true,
        stats: {
          reactions: 0,
          speed: uniqueStats.speed,
          combat: uniqueStats.combat,
          toughness: uniqueStats.toughness,
          savvy: 0,
          luck: uniqueEntry.luck,
        },
        ai: uniqueEntry.ai === "same" ? enemyTemplate.ai : uniqueEntry.ai,
        weapons: uniqueEntry.weapons.map((wId) => ({
          instanceId: `uw_${i}_${wId}`,
          weaponId: wId,
        })),
        isFearless: true,
        position: { x: -1, y: -1 },
  ...
  if (mission.type === "Eliminate") {
    const potentialTargets = participants.filter(
      (p) => p.type === "enemy" && !p.isUnique,
    );
  ...
  crewToPlace.forEach((char, index) => {
    const initialX =
      Math.floor(BATTLE_GRID_SIZE.width / 2) -
      Math.floor(crewToPlace.length / 2) +
      index;
    ...
    participants.push({
      ...
      portraitUrl: char.portraitUrl || getNextPortrait(),
    });
  });
  ...
  finalEnemies.forEach((enemy, index) => {
    const initialX =
      Math.floor(BATTLE_GRID_SIZE.width / 2) -
      Math.floor(finalEnemies.length / 2) +
      index;
    const initialPos = { x: initialX, y: 1 };
    enemy.position = findNearestWalkable(initialPos, {
      ...tempBattleForSetup,
      participants,
    });
  });
  ...
  battle.enemyTurnOrder = participants
  ...
  if (campaign?.currentWorld?.traits.some((t) => t.id === "haze")) {
    ...
    battle.log.push({
      key: "log.info.worldTraitHaze",
      params: { distance: hazeVisibility },
    });
  }
...
export const setupMultiplayerBattle = async (
  ...
  const missionTemplate = MISSION_DEFINITIONS.find(
    (m) => m.type === "FightOff",
  )!;
  const mission: Mission = {
    type: missionTemplate.type,
  ...
  const firstPlayerRole: MultiplayerRole =
  ...
  hostCrew.members.forEach((char, index) => {
    const initialX =
      Math.floor(BATTLE_GRID_SIZE.width / 2) -
      Math.floor(hostCrew.members.length / 2) +
      index;
    const initialPos = { x: initialX, y: BATTLE_GRID_SIZE.height - 2 };
  ...
  guestCrew.members.forEach((char, index) => {
    const initialX =
      Math.floor(BATTLE_GRID_SIZE.width / 2) -
      Math.floor(guestCrew.members.length / 2) +
      index;
    const initialPos = { x: initialX, y: 1 };
    const finalPos = findNearestWalkable(initialPos, battle);
    participants.push({
