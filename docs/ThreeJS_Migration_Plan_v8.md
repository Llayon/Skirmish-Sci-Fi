# План миграции на Three.js v8.0 (ФИНАЛЬНАЯ ПРОДАКШЕН-ГОТОВАЯ ВЕРСИЯ)
...
**Финальная версия:** Все критические ошибки исправлены, добавлен DTO-адаптер для корректной интеграции с доменными типами.

---

## Архитектура с DTO-адаптером

### Принцип разделения
- **Домен** (`Battle`, `Terrain`, `Participant`) — правила игры, состояние, логика
...
**`types/battle3d.ts`**
```typescript
import type { Position, GridSize } from './battle';

export interface Terrain3D {
  id: string;
  type: 'Wall' | 'Barrel' | 'Container' | 'Floor' | 'Obstacle';
  position: Position;
  height: number;
...
```typescript
import type { Battle, Terrain, Participant } from '@/types/battle';
...
function mapTerrainTo3D(terrain: Terrain): Terrain3D {
  // Маппинг доменных типов на 3D визуализацию
  const typeMapping: Record<string, Terrain3D['type']> = {
    'Wall': 'Wall',
    'Barricade': 'Wall',
    'Building': 'Wall',
    'Container': 'Container',
  ...
  return {
    id: terrain.id,
    type: typeMapping[terrain.type] || 'Floor',
    position: terrain.position,
    height: getTerrainHeight(terrain),
    blocksLineOfSight: terrain.blocksLineOfSight ?? false,
  };
}
...
function mapParticipantTo3D(
  ...
  // Вычисляем HP из доменного состояния
  // В TDD у участников stunTokens/currentLuck/status
  // Для 3D показываем "здоровье" как 100 - stunTokens * 20
  const hp = Math.max(0, 100 - (participant.stunTokens || 0) * 20);
  const maxHp = 100;

  return {
    id: participant.id,
    type: participant.type === 'character' ? 'character' : 'enemy',
...
function getTerrainHeight(terrain: Terrain): number {
  // Высота на основе типа
  const heightMapping: Record<string, number> = {
    'Wall': 2,
    'Barricade': 1.5,
    'Building': 3,
    'Container': 1.2,
  ...
  return heightMapping[terrain.type] || 0.1;
}
```
...
export const GridFloor: React.FC<GridFloorProps> = ({ gridSize }) => {
  const gridTexture = useMemo(() => {
    ...
    // Вертикальные линии
    for (let x = 0; x <= size; x += size / gridSize.width) {
      ctx.beginPath();
      ctx.moveTo(x, 0);
      ctx.lineTo(x, size);
      ctx.stroke();
    ...
    for (let y = 0; y <= size; y += size / gridSize.height) {
      ctx.beginPath();
      ctx.moveTo(0, y);
      ctx.lineTo(size, y);
      ctx.stroke();
...
function getTerrainMaterial(terrain: Terrain3D) {
  ...
  return (
    <meshStandardMaterial
      color={colors[terrain.type] || colors.Floor}
    />
  );
}
```
...
export const BattleView3D: React.FC<BattleView3DProps> = ({ battleLogic }) => {
  ...
  }, [battle, state.selectedId, state.activeParticipantId, derived.availableMoves, animation?.unitId]);
...
describe('TerrainMesh', () => {
  it('renders correct geometry for Wall terrain', () => {
