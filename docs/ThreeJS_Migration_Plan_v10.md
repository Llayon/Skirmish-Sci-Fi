# План миграции на Three.js v10.0 (ФИНАЛЬНАЯ ПРОДАКШЕН-ГОТОВАЯ ВЕРСИЯ)
...
**Финальная версия:** Все критические ошибки исправлены, адаптер корректно работает с доменными типами.
...
```typescript
import type { Battle, Terrain, Participant, Position, AnimationState } from '@/types/battle';
...
function guessVisualTypeFromFlags(t: Terrain): Terrain3D['type'] {
  ...
  // 5) fallback
  return t.providesCover ? 'Obstacle' : 'Obstacle';
}
...
function mapParticipantTo3D(
  ...
  // Определяем тип по принадлежности к crew (предполагаем что participant имеет crewId или аналог)
  // В вашем домене BattleParticipant должен иметь способ определения стороны
  const isCharacter = participant.type === 'character'; // или другое поле
...
describe('TerrainMesh', () => {
  it('renders correct geometry for Wall terrain', async () => {
    ...
    // Найти первый Mesh в сцене
    const mesh = renderer.scene.children.find(child => child.type === 'Mesh') as any;
