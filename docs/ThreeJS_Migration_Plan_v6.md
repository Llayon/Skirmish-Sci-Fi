# План миграции на Three.js v6.0 (ФИНАЛЬНАЯ ПРОДАКШЕН-ГОТОВАЯ ВЕРСИЯ)
...
export const AnimationSystem3D: React.FC<AnimationSystem3DProps> = ({
  ...
  switch (animation.type) {
    ...
    case 'shoot':
      console.warn('ShootAnimation not yet implemented');
      clearAnimation();
      return null;

    case 'hit':
    case 'death':
      console.warn(`${animation.type} animation not yet implemented`);
      clearAnimation();
      return null;
