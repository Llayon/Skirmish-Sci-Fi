# План миграции на Three.js v9.0 (ФИНАЛЬНАЯ ПРОДАКШЕН-ГОТОВАЯ ВЕРСИЯ)
...
**Финальная версия:** Все критические ошибки исправлены, добавлен DTO-адаптер с правильным маппингом доменных типов.
...
export interface Terrain3D {
  id: string;
  type: 'Wall' | 'Barrel' | 'Container' | 'Obstacle';
  position: Position;
  height: number;
...
```typescript
import type { Battle, Terrain, Participant, Position } from '@/types/battle';
...
export function mapBattleTo3D(
  ...
  return {
    ...
    terrain: battle.terrain
      .map(mapTerrainTo3D)
      .filter(t => t.type !== 'Floor'), // Не рендерим Floor как геометрию
...
function mapTerrainTo3D(terrain: Terrain): Terrain3D {
  // Маппинг по имени terrain, а не по типу (тип - это категория для правил)
  const visualMapping: Record<string, Terrain3D['type']> = {
    'Wall': 'Wall',
    'Barricade': 'Wall',
    'Building': 'Wall',
    'Container': 'Container',
  ...
  return {
    id: terrain.id,
    type: visualMapping[terrain.name] || 'Obstacle',
...
function getTerrainHeight(terrain: Terrain): number {
  ...
  return heightMapping[terrain.name] || 1;
}
```
