...
export type HudPreset = 'full' | 'tactical' | 'spectator' | 'custom';
  ...
  persist(
    ...
    })),
    {
      ...
      migrate: (persistedState) => {
        const state =
          persistedState as Partial<Pick<HudState, 'preset' | 'panels' | 'collapsed' | 'density'>> | undefined;
        ...
        if (preset === 'full' || preset === 'tactical' || preset === 'spectator') {
          return {
            preset,
